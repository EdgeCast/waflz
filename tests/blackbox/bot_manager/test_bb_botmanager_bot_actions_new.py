...
def setup_waflz_server_action():
    ...
    l_ja3_db_dir_new = l_data_file_path / "waf" / "db" / "bot_lmdb" / "new_bot_lmdb"
...
def setup_waflz_server_bot_lmdb():
    ...
    l_ja3_db_dir_new = os.path.realpath(os.path.join(l_file_path, '../../data/waf/db/bot_lmdb/new_bot_lmdb'))
...
def test_bot_score_from_lmdb(setup_waflz_server_bot_lmdb):
    ...
    assert l_r_json['bot_event']['customer_id'] == 50
    assert l_r_json['bot_event']['bot_tags'] == '[\"UniqueIpActivity\",\"Request Volume low\"]'
# ------------------------------------------------------------------------------
# test ja3 from lmdb
# ------------------------------------------------------------------------------
def test_bot_score_from_empty_ja3(setup_waflz_server_bot_lmdb):
    ...
    assert l_r_json['bot_event']['bot_tags'] == '[\"Url len inconsistency\"]'
...
def test_spoof_ip_gets_proper_bot_score(setup_waflz_server_bot_lmdb):
    ...
    assert l_r_json['bot_event']['customer_id'] == 51
