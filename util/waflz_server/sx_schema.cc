...
//! \return:  None
//! ----------------------------------------------------------------------------
sx_schema::sx_schema(ns_waflz::engine& a_engine)
    :m_engine(a_engine), m_schema(NULL), m_action(NULL)
{
        // -------------------------------------------------
...
//! ----------------------------------------------------------------------------
//! \details: Initialize sx_schema with m_config from sx superclass
//!           and create m_schema
//! \return:  WAFLZ Status Code
//! ----------------------------------------------------------------------------
int32_t sx_schema::init(void)
{
        // -------------------------------------------------
        // read file
        // -------------------------------------------------
        ...
        // load sx_schema
        // -------------------------------------------------
        m_schema = new ns_waflz::schema(m_engine);
        l_s = m_schema->load(l_buf, l_buf_len);
        if (l_s != WAFLZ_STATUS_OK)
        {
                NDBG_PRINT("error loading config: %s. reason: %s\n",
                           m_config.c_str(),
                           m_schema->get_err_msg());
                if (l_buf)
                {
                        free(l_buf);
                        l_buf = NULL;
...
//! \return:  IS2 Response
//! ----------------------------------------------------------------------------
ns_is2::h_resp_t sx_schema::handle_rqst(
...
{
        // -------------------------------------------------
        // if no schema - return error
        // -------------------------------------------------
        if (!m_schema) { return ns_is2::H_RESP_SERVER_ERROR; }
        // -------------------------------------------------
        // default response
        // -------------------------------------------------
        m_resp = "{\"status\": \"ok\"}";
        ...
        // sx_schema process
        // -------------------------------------------------
        int32_t l_s = l_ctx->init_phase_1(
            m_engine, NULL, NULL, NULL);
        if (l_s != WAFLZ_STATUS_OK)
        {
                ns_is2::H_RESP_SERVER_ERROR;
        }
        l_s = l_ctx->init_phase_2(m_engine.get_ctype_parser_map());
        if (l_s != WAFLZ_STATUS_OK)
        {
                ns_is2::H_RESP_SERVER_ERROR;
        }
        l_s = m_schema->process(&l_event, NULL, l_ctx->m_body_data, l_ctx->m_body_len);
        // -------------------------------------------------
