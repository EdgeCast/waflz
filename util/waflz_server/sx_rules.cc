...
//! \param:   TODO
//! ----------------------------------------------------------------------------
sx_rules::sx_rules(ns_waflz::engine& a_engine):
        m_engine(a_engine),
        m_rules(NULL),
        m_action(NULL)
{
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
sx_rules::~sx_rules(void)
{
        if(m_rules) { delete m_rules; m_rules = NULL; }
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
int32_t sx_rules::init(void)
{
        // -------------------------------------------------
        // load profile
        // -------------------------------------------------
        int32_t l_s;
        m_rules = new ns_waflz::rules(m_engine);
        //NDBG_PRINT("load rules: %s\n", m_config.c_str());
        l_s = m_rules->load_file(m_config.c_str(), m_config.length());
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
ns_is2::h_resp_t sx_rules::handle_rqst(waflz_pb::enforcement **ao_enf,
